			...
			if(r==0){
				...
				log("obj = " + glp_get_obj_val(lp));
						for(var i = 1; i <= glp_get_num_cols(lp); i++){
							...
							switch(r){
								case GLP_EBADB: log("Número de variáveis básicas não é o mesmo que o número de linhas do objeto do problema. ");
								break;
								
								case GLP_ESING: log("O modelo contém apenas uma matriz base dentro do modelo. ");
								break;
								
								case GLP_ECOND: log("Número de condição muito grande para a matriz base inicial. ");
								break;
								
								case GLP_EBOUND: log("Variáveis limitadas reais com limites incorretos. ");
								break;
								
								case GLP_EFAIL: log("A busca da resposta foi encerrada devido a falha do solver. ");
								break;
								
								case GLP_EOBJLL: log("A função objetivo que era pra ser maximizada atingiu seu menor valor e continua diminuindo. ");
								break;
								
								case GLP_EOBJUL: log("A função objetivo que era pra ser minimizada atingiu seu maior valor e continua aumentando. ");
								break;
								
								case GLP_EITLIM: log("A iteração do simplex excedeu o limite. ");
								break;
								
								case GLP_ETMLIM: log("O tempo limite foi excedido. ");
								break;
								
								case GLP_ENOPFS: log("Não tem solução viável primal. ");
								break;
								
								case GLP_ENODFS: log("Não tem solução viável dual. ");	
								break;		
				...
				for(var i = 1; i <= glp_get_num_cols(lp); i++){
					log(glp_get_col_name(lp, i)  + " = " + glp_mip_col_val(lp, i));
						...
						switch(r){
									...
									case GLP_EBOUND: log("Algumas variáveis reais ou inteiras estão com os limites incorretos. ");
									case GLP_EROOT: log("Base ideal para o problema de PL não é fornecido. ");
									
									case GLP_ENOPFS: log("Não tem solução viável primal. ");
									
									case GLP_ENODFS: log("Não tem solução viável dual, há pelo menos uma solução viável primitiva, a solução é ilimitada. ");
									
									case GLP_EFAIL: log("Falha do solver. ");
									
									case GLP_EMIPGAP: log("");
